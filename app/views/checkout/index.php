...
<div class="border-t pt-4 space-y-2 text-sm">
    ...
    <div class="flex justify-between font-bold text-base text-orange-500">
        ...
        <span id="total-text">
            Rp <?= number_format($cart['total_price'] + $ongkir,0,',','.') ?>
        </span>
    </div>
</div>
...
function hitungOngkir(kota) {
    ...
    if (kota.includes('jakarta')) return 10000;
    if (kota.includes('bandung')) return 15000;
    if (kota.includes('surabaya')) return 20000;
    return 25000;
}
...
function updateTotal(ongkir) {
    ...
    ongkirText.dataset.value = ongkir;
...
if (cityInput.value.trim() !== '') {
    const ongkir = hitungOngkir(cityInput.value);
    updateTotal(ongkir);
}

// 🔁 HITUNG SAAT KOTA DIUBAH
cityInput.addEventListener('change', function () {
    const ongkir = hitungOngkir(this.value);
    updateTotal(ongkir);
});
</script>
